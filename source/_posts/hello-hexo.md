...
### 部署hexo
因为写完文章之后，使用 `hexo g` 每次会生成新的 html 文件，如果使用git命令来同步有点麻烦，hexo 提供了直接部署的功能，只要在项目 `_config.yml` 中进行配置。因为前面已经安装 **Git Desktop** 运行登录过,所以在我这边操作的时候好像不需要设置 **SSH Key** ，我这里就简单的配置下就可以部署，下面是我的配置参数
```javascript
deploy:
  type: git
